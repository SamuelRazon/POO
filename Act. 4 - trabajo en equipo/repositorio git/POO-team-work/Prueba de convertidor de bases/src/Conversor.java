...
public class Conversor {
    /**
     * Transformar numero decimal a numero con otra base
     ...
     */
    public static String convertirBase(String numero, int baseOrigen,int baseDestino){
        ...
        if (baseDestino <2 || baseDestino >35){ // Verificar que la base este entre 2 y 35
            return "ERROR: El programa solo calcula bases entre 2 y 35";
        }
        if(!validacion(numero, baseOrigen)){    // Validar que la entrada coincida con la base
            return "ERROR: El número no coincide con la base numérica";
        }
       ...
       /////*Transformar decimal a número de la base esperada */////
        ...
        while (intNumero > 0){
            digits.add(/*residuo = */intNumero%baseDestino);
     ...
     */
    private static boolean validacion (String numero, int baseOrigen){
        if(baseOrigen <= 10){//Cuando la base del número es de 10 o mayor
            return numero.matches("[0-" + String.valueOf(baseOrigen - 1) + "]+");
        }
        else{//Cuando la base es mayor a 10
            return numero.matches("[0-9A-"+String.valueOf((char)(baseOrigen+54))+"]+");
        }
    }
}
